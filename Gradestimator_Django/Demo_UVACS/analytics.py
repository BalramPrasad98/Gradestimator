...
from .models import SignificantCourse
...
def runanalytics(myCourses):
    ...
    # If there is more than 1 significant course then subset for those courses as well
    if len(Selected_Courses)>1:
        for i in range(1,len(Selected_Courses)):
            ...
            temp_student = sub_frame['Student'][(sub_frame['Course'] == second_course) &
            (sub_frame['Grade'] == second_grade)]
